	...
	m_deviceResources(deviceResources)
{
	...
	DX::ThrowIfFailed(
		m_deviceResources->GetD2DFactory()->CreateDrawingStateBlock(winrt::put_abi(m_stateBlock))
		);
...
void SampleFpsTextRenderer::Update(DX::StepTimer const& timer)
{
	...
	DX::ThrowIfFailed(
		m_deviceResources->GetDWriteFactory()->CreateTextLayout(
			...
			winrt::get_abi(m_textFormat),
			240.0f, // Max width of the input text.
			50.0f, // Max height of the input text.
			winrt::put_abi(textLayout)
			)
		);
...
void SampleFpsTextRenderer::Render()
{
	...
	context->SaveDrawingState(winrt::get_abi(m_stateBlock));
	context->BeginDraw();
	...
	context->DrawTextLayout(
		...
		winrt::get_abi(m_textLayout),
		winrt::get_abi(m_whiteBrush)
		);
	...
	context->RestoreDrawingState(winrt::get_abi(m_stateBlock));
}

void SampleFpsTextRenderer::CreateDeviceDependentResources()
{
	DX::ThrowIfFailed(
		m_deviceResources->GetD2DDeviceContext()->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::White), winrt::put_abi(m_whiteBrush))
		);
}
